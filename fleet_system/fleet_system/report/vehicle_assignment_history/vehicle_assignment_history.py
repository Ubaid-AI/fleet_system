...
def get_vehicle_history(filters):
	...
	for idx, row in enumerate(data):
		...
		# with open('/home/frappe/frappe-bench/apps/erpnext/erpnext/fleet_ms/report/vehicle_assignment_history/t.txt', 'w+') as f:
		# 	f.writelines(str(changed_list))
		if changed_list:
			for changes in changed_list:
				if 'employee' in changes:
					data[idx]['modified_field'] = changes[0].replace('_', ' ').title()
					if changes[1]:
						data[idx]['previous_value'] = frappe.db.sql(f"""Select e.employee_name from tabEmployee e WHERE e.employee =  '{changes[1]}' """)[0]
					else:
						data[idx]['previous_value'] = 'None'
					data[idx]['new_value'] = frappe.db.sql(f"""Select d.employee_name from tabEmployee d WHERE d.employee =  '{changes[2]}' """)[0][0]
